...
ARAPDeformer::ARAPDeformer(Surface_mesh * usingMesh)
{
	...
	isAnchorPoint = mesh->vertex_property< bool >("v:anchorPoint");
	isControlPoint = mesh->vertex_property< bool >("v:controlPoint");
...
void ARAPDeformer::ComputeCotWeights()
{
	...
	for (vit = mesh->vertices_begin(); vit != vend; ++vit)
	{
		...
		do{ 
			...
			if(!mesh->is_boundary(mesh->find_halfedge(pid, p2id))
				&&!mesh->is_boundary(mesh->find_halfedge(p2id, pid))) // not a boundary edge
			...
			else // boundary edge, only have one such angle
			{
				...
				{
					if(!mesh->is_boundary(mesh->find_halfedge(pid, p1id))
						&&!mesh->is_boundary(mesh->find_halfedge(p1id, pid))) 
						wij = dot((p-p1),(p2-p1)) / cross((p-p1),(p2-p1)).norm();
					else
	...
	for (vit = mesh->vertices_begin(); vit != vend; ++vit)
	{
		...
		if(!(isAnchorPoint[vit] || isControlPoint[vit]))
		{
			...
			do{
				...
				A.coeffRef(i,j) = -wij_weight[vit][vvit];
			} while(++vvit != vvend);
		}
		else
			weight = 1.0;
